...
def start(message):
    ...
    text_message2 = f'Этот бот предназначен для выполнения математических операций,\n '\
                    f'а также же просмотра расслабляющего видео и помощи!'
...
def mess(message):
    ...
    if 'start' in get_message_bot:
        ...
        try:
            ...
            json_work.write(message.chat.id, f'{get_message_bot} = {calc}', message.from_user.first_name, reply_markup=markup)
...
def history(message):
    get_message_bot = message.text.strip().lower()
    if 'Посмотреть историю' in get_message_bot:
